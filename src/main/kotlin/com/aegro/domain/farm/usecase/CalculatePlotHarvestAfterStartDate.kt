...
@Named
class CalculatePlotHarvestAfterStartDate(
    ...
    override suspend fun execute(farmId: String, plotId: String, startDate: LocalDate?, endDate: LocalDate?): Result<Int, Exception> {
        ...
        return findPlotHarvestsProductivityAfterStartDate(plots, plotId, startDate)
                ?.let {
                    Success(it)
    ...
    private fun findPlotHarvestsProductivityAfterStartDate(plots: List<Plot>?, plotId: String, startDate: LocalDate?) =
